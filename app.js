...
var systemLoanApproval = cron.schedule('0 * * * * *', function() {
	console.log('Cron is to fetch credit score of applicatns');
	process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
	function123();
}, true);
systemLoanApproval.start();
require('./config.js');
...
app.use(expressJWT({
	...
	path: ['/users','/login','/registerUser']
}));
app.use(bearerToken());
app.use(function(req, res, next) {
	if (req.originalUrl.indexOf('/users') >= 0 || req.originalUrl.indexOf('/login') >= 0 || req.originalUrl.indexOf('/registerUser') >= 0) {
		return next();
	}
...
app.post('/registerUser', function(req, res, next) {
	...
	console.log(firstname,lastname,username,password,role)	// values ('sarita.t','xoriant12345','sarita','tompe','USER')
	var query = 'INSERT INTO users(username,password,firstname,lastname,role) VALUES ("'+ username + '", "' + password + '", "' + firstname + '", "' + lastname + '", "' + role +'")';
	connection.query(query, function (err, rows, fields) {
	if (err) throw err
	res.json(rows)
})
